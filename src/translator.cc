...
namespace ctranslate2 {
    ...
    std::vector<TranslationResult>
    Translator::translate_batch_with_prefix_bk(const std::vector<std::vector<std::string>> &source,
    ...
    Translator::translate_batch_with_prefix(const std::vector<std::vector<std::string>> &source,
        ...
        if (options.decode_with_fsa_prefix) {
            if (options.return_attention)
                throw std::invalid_argument(
                        "decode_with_fsa_prefix currently does not support returning attention vectors");
            if (batch_size > 1)
                throw std::invalid_argument(
                        "decode_with_fsa_prefix currently does not support batch inputs");
            if (options.beam_size > 1)
                throw std::invalid_argument(
                        "decode_with_fsa_prefix currently does not support beam_search, only greedy search");
            /*
            if (_model->device() == Device::CPU)
                throw std::invalid_argument(
                        "decode_with_fsa_prefix currently does not support Device::CPU");
            */
            if (with_prefix)
                throw std::invalid_argument(
                        "decode_with_fsa_prefix currently does not support ctranslate2's with_prefix");
            /*
            if (options.min_decoding_length > 0) {
                throw std::invalid_argument(
                        "With decode_with_fsa_prefix, options.min_decoding_length should equal to 0.");
            }*/
    ...
    std::vector<TranslationResult>
    Translator::run_translation_bk(const std::vector<std::vector<std::string>> &source,
                                const std::vector<std::vector<std::string>> &target_prefix,
                                const TranslationOptions &options) {
    ...
    Translator::run_translation(const std::vector<std::vector<std::string>> &source,
                                ...
                                const TranslationOptions &options) {
        /*
        std::cout << "Inside run_translation \n";
        std::cout << "beam_size " << options.beam_size << "\n";
        std::cout << "decode_with_fsa_prefix " << options.decode_with_fsa_prefix << "\n";
        */
        ...
        if (options.beam_size == 1)
            if (options.decode_with_fsa_prefix)
                greedy_search_with_fsa_prefix(decoder,
                              state,
                              *sampler,
                              sample_from,
                              candidates,
                              encoded,
                              lengths,
                              *options.emission_matrix,
                              *options.transition_matrix,
                              *options.length_matrix,
                              options.init_state,
                              start_step,
                              end_token,
                              options.max_decoding_length,
